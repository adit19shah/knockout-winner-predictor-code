#include<bits/stdc++.h>
using namespace std;

#define int long long 
#define fast ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0)
#define precise(x) fixed << setprecision(x) 
 

int pow3[20]; 
int fact[20];
int ways[20];
double prob[20][20]; 
double ans[20];

map<int,vector<int>> vm;

int count_set_bits(int mask)
{
    int i,j,cnt;
    cnt=0;
    while(mask!=0)
    {
        if(mask%2==1)
        {
            cnt++;
        }
        mask=mask/2;
    }
    return cnt;
}


bool valid_Mask(int mask)
{
    //The number of set bits in the mask should be power of 2
    int i,j;

    // i is number of set bits
    i=__builtin_popcount(mask);

    if(i==0)
    {
        return 0;
    }
    
    while(i>1)
    {
        if(i%2==1)
        {
            //number of set bits not a power of 2
            return 0;
        }
        i=i/2;
    }
    
    // mask is a valid mask
    return 1;
}


bool valid_Submask(int mask,int submask,int n)
{
    int i,j,cnt1,cnt2;
    
    for(i=0;i<n;i++)
    {
        if((mask&(1<<i))==0 && (submask&(1<<i))!=0)
        {
            // submask cant be generated from given mask
            return 0;
        }
    }

    //cnt1 and cnt2 are number of set bits in mask and submask respectively 
    cnt1=count_set_bits(mask);
    cnt2=count_set_bits(submask);

    //half the players win and half of them loose
    if(cnt1==2*cnt2)
    {
        //shows valid submask
        return 1;
    }

    //shows invalid submask
    return 0;
}


int get_Bit(int mask,int i)
{
    // this function is for getting value of bit in dp2
    // normal bitwise operators don't work
    // since we have 3 possible values - 0,1 and 2 

    int b;
    mask=mask%pow3[i+1];
    b=mask/pow3[i];
    return b;
}


double calculate_Probability(int mask,int n)
{
    int i,j,k,b1,b2,b;

    //cout<<". ";
    if(mask==0)
    {
        return 1.00;
    }

    double p=0.0;
    int new_mask;
    
    for(i=n-1;i>=0;i--)
    {
        b1=get_Bit(mask,i); // last losing bit
        if(b1==2)
        {
            for(j=0;j<n;j++)
            {
                b2=get_Bit(mask,j); //all winning bits
                if(b2==1)
                {
                    new_mask=mask-1*pow3[j]-2*pow3[i];
                    p+=prob[j][i]*calculate_Probability(new_mask,n);
                }
            }
            break;
        }
    }

    
    return p;
}


double Prob_Mask_To_Submask(int mask,int submask,int n)
{
    int i,j,k;
    double p; // stores final probability

    // 0 bit --> team already eliminated
    // 1 bit --> team expected to win
    // 2 bit --> team expected to lose

    int new_mask=0;
    for(i=0;i<n;i++)
    {
        if(((mask&(1<<i))!=0) && ((submask&(1<<i))!=0)) 
        {
            //team i wins
            new_mask+=1*pow3[i];
        }
        else if(((mask&(1<<i))!=0) && ((submask&(1<<i))==0)) 
        {
            //team i loses
            new_mask+=2*pow3[i];
        }
        else 
        {
            //team i has already lost
            new_mask+=0*pow3[i];
        }
    }
    
    p=calculate_Probability(new_mask,n);
    return p;

}


void gen_mask(int mask,int n)
{
    int i=mask,j,k,m;
    
    for(j=i;j;j=(j-1)&i)
    {
        if(valid_Submask(i,j,n)==1)
        {
           vm[j].push_back(i);
        }
    }
    
}

double rec(int submask,int n)
{
    double ans_rec;
    
    if(submask==(1<<n)-1)
    {
        ans_rec=1.00;
    }
    else
    {
        int i,j,k,sz,m;
        double d;
        ans_rec=0;
        sz=vm[submask].size();
        for(i=0;i<sz;i++)
        {
            m=__builtin_popcountll(submask);
            k=ways[m];
            d=1.00/(double)k;
            ans_rec+=d*rec(vm[submask][i],n)*Prob_Mask_To_Submask(vm[submask][i],submask,n);
        }
    }
    return ans_rec; 
}

void solve(int n)
{
    int i,j,k,m;
    double d;
    vm.clear();

    //store powers of 3s for future use
    pow3[0]=1;
    for(i=1;i<=n;i++)
    {
        pow3[i]=pow3[i-1]*3;
    }

    //storing ways to play games with 2*i teams for future use
    ways[1]=1;
    for(i=2;i<=n/2;i++)
    {
        ways[i]=ways[i-1]*(2*i-1);
    }

    //storing factorial for future use
    fact[0]=1;
    for(i=1;i<=n;i++)
    {
        fact[i]=fact[i-1]*i;
    }
    

    // 0 bit --> team already eliminated
    // 1 bit --> team still in tournament
    for(i=(1<<n)-1;i>=0;i--)
    {
        gen_mask(i,n);
    }

    // store final answer 
    for(i=0;i<n;i++)
    {
        ans[i]=rec((1<<i),n);
    }
    
}

signed main()
{
    //for fast input output
    fast;
    //give ouputs with precision of 4
    cout<<precise(4);

    
    
    int i,j,k,n;
    //input number of teams
    cin>>n;
    
    //give input probability
    //prob[i][j]=probability of winning for team i over team j
    for(i=0;i<n;i++)
    {
        for(j=0;j<n;j++)
        {
            cin>>prob[i][j];
        }
    }

    
    cout<<"The number of teams are "<<n<<"."<<endl;
    cout<<endl;
    
    cout<<"The probability table is as follows: "<<endl;
    for(i=0;i<n;i++)
    {
        for(j=0;j<n;j++)
        {
            cout<<prob[i][j]<<" ";
        }
        cout<<endl;
    }
    cout<<endl;
    

    //solve the question for n teams with given probability table
    solve(n);
    
    //print answer
    for(i=0;i<n;i++)
    {
        cout<<"Probability of team "<<i+1<<" winning the tournament is "<<ans[i]<<"."<<endl;
    }
    
}